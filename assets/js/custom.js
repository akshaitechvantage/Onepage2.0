(function ($) {
  ...
  function dsnAjax() {
    ...
    return {
      ...
      effectAjax: function ($add) {
        if ($add) $("html").addClass("dsn-ajax-effect");
        else if ($add === false) $("html").removeClass("dsn-ajax-effect");
        else return $("html").hasClass("dsn-ajax-effect");
      },
      ...
      ajaxSlider: function ($e) {
        let active = $e.parents(".slide-content"),
          ...
          img = active
            .parents(".main-slider")
            .find('.swiper-slide[data-dsn-id="' + id + '"] .image-bg')
            .first(),
  ...
  function magnificPopup() {
    ...
    $(".dsn-stories .dsn-stories-gallery").each(function () {
      ...
      pop_first.append(
        "<img src='" + pop_first.attr("href") + "' class='cover-bg-img'>"
      );

      $(this).magnificPopup(option);
      dsnGrid.convertTextLine(
        $(this).find(".heading-h2 , .sm-title-block , .title-block")
      );
    });
  }
    ...
    return {
      ...
      nextProject: function () {
        ...
        if (_next.find(".w-100.pt-30.d-flex.justify-content-between").length) {
          tween.from(
            _next.find(".w-100.pt-30.d-flex.justify-content-between"),
            {
              autoAlpha: 0,
              ease: "none",
            },
        ...
        $(".dsn-header-animation").each(function () {
          ...
          if (heroTitle.length) {
            ...
            parallax.to(
              ...
              {
                y: fixed
                  ? heroTitle.parent().height() -
                    heroTitle.height() -
                    heroTitle.offset().top -
                    space
                  : 150,
                yoyo: true,
                autoAlpha: fixed ? 1 : 0,
      ...
      dsnScrollTop: function () {
        ...
        parallaxIt.on("progress", function ($s) {
          $(".scroll-to-top .box-number span").text(
        ...
        if (dsnGrid.isMobile()) {
          ...
          parallaxIt.on("leave", function () {
            ...
            } else parallaxIt.duration(dsnGrid.getHeightScroll());
          });
        } else {
          parallaxIt.on("enter", function () {
          ...
          on: {
            init: function () {
              ...
              dsn_slider
                ...
                .html(dsnGrid.numberText(swiper.slides.length));
      ...
      slideChange: function (swiper, dsn_slider, $dir) {
        ...
        async function start() {
          ...
          dsn_slider
            ...
            .html(dsnGrid.numberText(swiper.activeIndex + 1));
    ...
    setTimeout(function () {
      $("[data-dsn-" + $type + "]").each(function () {
        if ($type === "bg")
          $(this).css(
            "background-image",
            `url(${dsnGrid.getData(this, $type, "")})`
          );
        else $(this).attr($type, dsnGrid.getData(this, $type, ""));
      });
    }, 100);
  }
